...
export default function ImageReorder({ images: initial = [], projectId, onReordered }: Props) {
  const [items, setItems] = useState<ImageRead[]>([...initial].sort((a,b)=>a.ordering - b.ordering))
      ...
      <div className="grid grid-cols-3 gap-2">
        {items.map((img, idx) => (
          <div key={img.id} draggable onDragStart={(e)=>onDragStart(e, idx)} onDragOver={onDragOver} onDrop={(e)=>onDrop(e, idx)} className="border p-1">
            <img src={`${img.public_url}`} className="w-full h-32 object-cover" alt={img.caption || ''} />
            <div className="text-xs mt-1">#{img.id} • order: {idx}</div>
          </div>
        ))}
      </div>
      <div className="mt-3">
        <button className="bg-blue-600 text-white px-3 py-1 rounded" onClick={saveOrder}>Save order</button>
      </div>
    </div>
  )
}
