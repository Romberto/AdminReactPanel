...
export default function ImageUploader({ projectId, onUploaded }: Props) {
  ...
  const uploadAll = async () => {
    for (const file of files) {
      ...
      // so we'll use native fetch with FormData to show progress.
      const url = `${import.meta.env.VITE_API_URL}/api/v1/admin/projects/${projectId}/images`
  ...
  return (
    <div className="p-2 border rounded">
          ...
          {previews.map((p, i) => (
            <div key={p} className="relative">
              ...
              <div className="h-2 bg-slate-200 rounded mt-1">
                <div style={{ width: `${Math.max(0, progress[files[i].name] || 0)}%` }} className="h-2 bg-green-500 rounded" />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-3 flex gap-2">
        <button className="bg-blue-600 text-white px-3 py-1 rounded" onClick={uploadAll} disabled={files.length===0}>Upload</button>
        <button className="px-3 py-1 rounded border" onClick={() => { setFiles([]); setPreviews([]) }}>Clear</button>
      </div>
    </div>
  )
}
