...
export const projectsApi = api.injectEndpoints({
  endpoints: build => ({
    ...
    updateProject: build.mutation<
      ProjectRead,
      { project_id: string; body: ProjectUpdate }
    >({
    ...
    uploadImage: build.mutation<
      ImageRead,
      { project_id: string; public_url: string; path_to_file: string }
    >({
    ...
    deleteImage: build.mutation<
      { message: string },
      { project_id: string; image_id: string }
    >({
      query: ({ project_id, image_id }) => ({
        url: `/api/v1/admin/projects/${project_id}/images/${image_id}`,
        method: 'DELETE',
      }),
      invalidatesTags: ['Images', 'Projects'],
    }),
    reorderImages: build.mutation<
      { message: string },
      { project_id: string; orders: Record<number, number> }
    >({
      query: ({ project_id, orders }) => ({
        url: `/api/v1/admin/projects/${project_id}/images/reorder`,
        method: 'POST',
        body: orders,
      }),
      invalidatesTags: ['Images', 'Projects'],
    }),
    isPreviewImages: build.mutation<
      { message: string },
      { project_id: string; image_id: string }
    >({
      query: ({ project_id, image_id }) => ({
        url: `/api/v1/admin/projects/${project_id}/images/ispreview/${image_id}`,
        method: 'POST',
      }),
      invalidatesTags: ['Images', 'Projects'],
    }),
